...
void bc_disassemble(const Bytecode* bc) {
    ...
    while (ip < bc->length) {
        ...
        switch (opcode) {
            ...
            case OP_JUMP_8: {
                int8_t offset = (int8_t)bc->code[ip + 1];
                printf("JUMP_8 %+d -> %zu", offset, ip + 2 + offset);
                break;
            }
            ...
            case OP_JUMP_IF_FALSE_8: {
                int8_t offset = (int8_t)bc->code[ip + 1];
                printf("JUMP_IF_FALSE_8 %+d -> %zu", offset, ip + 2 + offset);
                break;
            }
            
            case OP_CALL_8: {
                uint8_t args = bc->code[ip + 1];
                uint8_t addr = bc->code[ip + 2];
                printf("CALL_8 args=%u addr=%u", args, addr);
                break;
            }
